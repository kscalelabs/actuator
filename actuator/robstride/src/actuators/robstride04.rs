...
pub struct RobStride04Feedback {
    pub angle_rad: f32,
    pub velocity_rads: f32,
    pub torque_nm: f32,
}

impl TypedCommandData for RobStride04Command {
...
impl Actuator for RobStride04 {
    ...
    async fn set_max_torque(&self, torque: f32) -> Result<()> {
        let param = RobStride04Parameter::LimitTorque;
    ...
    async fn set_max_velocity(&self, velocity: f32) -> Result<()> {
        let param = RobStride04Parameter::LimitSpd;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: velocity,
    ...
    async fn set_max_current(&self, current: f32) -> Result<()> {
        let param = RobStride04Parameter::LimitCur;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: current,
...
impl ActuatorParameter for RobStride04Parameter {
...
impl RobStride04Parameter {
    pub fn iter() -> impl Iterator<Item = RobStride04Parameter> {
        use RobStride04Parameter::*;
        [
            RunMode,
            IqRef,
            SpdRef,
