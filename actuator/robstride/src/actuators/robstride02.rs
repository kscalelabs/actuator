...
pub struct RobStride02Feedback {
    pub angle_rad: f32,
    pub velocity_rads: f32,
    pub torque_nm: f32,
}

impl TypedCommandData for RobStride02Command {
...
impl TypedFeedbackData for RobStride02Feedback {
...
impl Actuator for RobStride02 {
    ...
    async fn set_max_torque(&self, torque: f32) -> Result<()> {
        let param = RobStride02Parameter::LimitTorque;
    ...
    async fn set_max_velocity(&self, velocity: f32) -> Result<()> {
        let param = RobStride02Parameter::LimitSpd;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: velocity,
    ...
    async fn set_max_current(&self, current: f32) -> Result<()> {
        let param = RobStride02Parameter::LimitCur;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: current,
...
impl ActuatorParameter for RobStride02Parameter {
...
impl RobStride02Parameter {
    pub fn iter() -> impl Iterator<Item = RobStride02Parameter> {
        use RobStride02Parameter::*;
        [
            RunMode,
            IqRef,
            SpdRef,
