...
pub struct RobStride00Feedback {
    pub angle_rad: f32,
    pub velocity_rads: f32,
    pub torque_nm: f32,
}

impl TypedCommandData for RobStride00Command {
...
impl TypedFeedbackData for RobStride00Feedback {
...
impl Actuator for RobStride00 {
    ...
    async fn set_max_torque(&self, torque: f32) -> Result<()> {
        let param = RobStride00Parameter::LimitTorque;
    ...
    async fn set_max_velocity(&self, velocity: f32) -> Result<()> {
        let param = RobStride00Parameter::LimitSpd;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: velocity,
    ...
    async fn set_max_current(&self, current: f32) -> Result<()> {
        let param = RobStride00Parameter::LimitCur;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: current,
...
impl ActuatorParameter for RobStride00Parameter {
...
impl RobStride00Parameter {
    pub fn iter() -> impl Iterator<Item = RobStride00Parameter> {
        use RobStride00Parameter::*;
        [
            RunMode,
            IqRef,
            SpdRef,
