...
pub struct RobStride03Feedback {
    pub angle_rad: f32,
    pub velocity_rads: f32,
    pub torque_nm: f32,
}

impl TypedCommandData for RobStride03Command {
...
impl TypedFeedbackData for RobStride03Feedback {
...
impl Actuator for RobStride03 {
    ...
    async fn set_max_torque(&self, torque: f32) -> Result<()> {
        let param = RobStride03Parameter::LimitTorque;
    ...
    async fn set_max_velocity(&self, velocity: f32) -> Result<()> {
        let param = RobStride03Parameter::LimitSpd;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: velocity,
    ...
    async fn set_max_current(&self, current: f32) -> Result<()> {
        let param = RobStride03Parameter::LimitCur;
        let cmd = WriteCommand {
            host_id: self.host_id,
            parameter_index: param.metadata().index,
            data: current,
...
impl ActuatorParameter for RobStride03Parameter {
...
impl RobStride03Parameter {
    pub fn iter() -> impl Iterator<Item = RobStride03Parameter> {
        use RobStride03Parameter::*;
        [
            RunMode,
            IqRef,
            SpdRef,
